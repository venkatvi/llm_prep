...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, kv: Optional[torch.Tensor], expanding_context: bool) -> torch.Tensor:
        ...
        assert (kvD % self.num_heads == 0), "KV Embed Dim should be divisible by num_heads"
        ...
        k: torch.Tensor = (
            self.k_proj(kv).reshape([kvB, kvS, self.num_heads, kv_head_dim]).permute(0, 2, 1, 3)
        )
        v: torch.Tensor = (
            self.v_proj(kv).reshape([kvB, kvS, self.num_heads, kv_head_dim]).permute(0, 2, 1, 3)
        )
    ...
    def forward_with_cache(self, input: torch.Tensor, kv: Optional[torch.Tensor], expanding_context: bool) -> torch.Tensor:
        ...
        if self.kv_cache is None: 
            ...
            k_new: torch.Tensor = self.k_proj(kv[:, -1, :]).reshape([kvB, 1, self.num_heads, self.head_dim]).permute(0, 2, 1, 3)
            v_new: torch.Tensor = self.v_proj(kv[:, -1, :]).reshape([kvB, 1, self.num_heads, self.head_dim]).permute(0, 2, 1, 3)
            
            # k_new, v_new [kvB, num_heads, kvS, head_dim]
            all_k: torch.Tensor = torch.cat([self.kv_cache["key"][:, :, :, :], k_new], dim=2)
            all_v: torch.Tensor = torch.cat([self.kv_cache["value"][:, :, :, :], v_new], dim=2)
