...
class GroupQueryAttention(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        q = self.q_proj(input).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # [bs, num_heads, seq_len, head_dim]
        k = self.k_proj(input).reshape(batch_size, seq_len, self.num_groups, self.head_dim).permute(0, 2, 1, 3)  # [bs, num_groups, seq_len, head_dim]
        v = self.v_proj(input).reshape(batch_size, seq_len, self.num_groups, self.head_dim).permute(0, 2, 1, 3)  # [bs, num_groups, seq_len, head_dim]
