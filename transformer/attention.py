...
def scaled_dot_product_attention(
    ...
    scores: torch.Tensor = q @ k.transpose(-2, -1)  # bs, n_heads, seq_len, head_dim @ bs, n_heads, head_dim, seq_len
    ...
    attn_out: torch.Tensor = scores @ v  # bs, n_heads, seq_len, seq_len @ bs, n_heads, seq_len, head_dim --> bs, n_heads, seq_len, head_dim
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor: 
        ...
        k: torch.Tensor = self.k_proj(input).reshape([input.size(0), input.size(1), self.num_heads, self.head_dim]).permute(0, 2, 1, 3)
        v: torch.Tensor = self.v_proj(input).reshape([input.size(0), input.size(1), self.num_heads, self.head_dim]).permute(0, 2, 1, 3)
