...
class WordTopKMapReduce(WordCountMapReduce):
    """MapReduce operations for word frequency counting."""
    ...
    @staticmethod
    def reduce_all(
        all_files_word_stats: list[dict[str, int]], use_reduce: bool = False, top_k: int = 10
    ) -> dict[str, int]:
        """Aggregate word counts across multiple files."""
        word_stats = defaultdict(int)
        if use_reduce:
        ...
        top_k = dict(sorted(word_stats.items(), key=lambda x:x[1], reverse=True)[:top_k])
        return top_k



class FrequencyCountMapReduce(WordCountMapReduce):
    """MapReduce operations for word frequency counting."""
    ...
    def reduce_all(
        ...
        # transform this into look up of unique frequencies and number of words which appear with that frequency 
        sorted_frequencies = sorted(word_stats.items(), key=lambda x: x[1])
        freq_dict = defaultdict(int)
        for word_freq_tuple in sorted_frequencies: 
            freq_dict[word_freq_tuple[1]] += 1 
        
        return dict(sorted(freq_dict.items(), key=lambda x:x[0]))
